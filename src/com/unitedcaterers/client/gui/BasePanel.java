...
/**
 * This class implements the generic utility functions that are needed by all the Views (Panels). So all the Panels such as
 ...
 * implemented by the actual Panel class itself.
 * This class also implements KeyListener interface which will be useful in future enhancements for adding Help Support (F1).
 *
 ...
 * there could be a menu item that hides or unhides a particular component on a Panel. When the user clicks on this menu
 * item, there may not be any need for the Panel to send this event to the controller because the Panel can take
 * appropriate action and change the view without any help of the controller. Therefore, in this case, the Panel will
 * not send this even to the controller.
 * On the other hand, when the user clicks a  "Search" button, the Panel has to send
 ...
 * Thus, events are actually handled by the Panel itself but in certain cases where it is necessary for the panel to
...
public abstract class BasePanel extends JPanel  implements Observer, KeyListener {
    ...
    private ArrayList<ActionListener> userlisteners;
    /**
    ...
    public void keyPressed(KeyEvent ke) {
        System.out.println("Caught key pressed event. Not implemented in this project but can be used to provide Help on F1.");
    }
    @Override
    ...
    protected void postUserActionEvent(ActionEvent viewevent) {
        ArrayList list = (ArrayList)userlisteners.clone(); //typically, when events are delivered to each listener, we don't want the list of listeners to be updated at the same time. So we clone the list and send events to all the listeners in the cloned list.
